  ...
  "node_modules/acorn/dist/acorn.js"(exports, module) {
    (function(global, factory) {
      ...
      pp2.fullCharCodeAt = function(pos) {
        var code4 = this.input.charCodeAt(pos);
...
function visit(tree, visitor) {
  ...
  function build(node2, key2, index2, parents) {
    ...
    return visit3;
    function visit3() {
      const result = enter ? toResult(enter(node2, key2, index2, parents)) : [];
      if (result[0] === EXIT) {
        return result;
      }
      if (result[0] !== SKIP) {
...
function factoryMdxExpression(effects, ok3, type, markerType, chunkType, acorn, acornOptions, addResult, spread, allowEmpty, allowLazy) {
  ...
  function before(code4) {
    ...
    if (code4 === 125 && size === 0) {
      const next = acorn ? mdxExpressionParse.call(self2, acorn, acornOptions, chunkType, eventStart, pointStart2, allowEmpty || false, spread || false) : {
        type: "ok",
        estree: void 0
      };
      if (next.type === "ok") {
  ...
  function eolAfter(code4) {
    ...
    if (markdownSpace(code4)) {
      return factorySpace(effects, before, "linePrefix", indentSize + 1)(code4);
    }
    return before(code4);
  }
}
function mdxExpressionParse(acorn, acornOptions, chunkType, eventStart, pointStart2, allowEmpty, spread) {
  ...
  if (spread && estree) {
    ...
    if (head2.type !== "ExpressionStatement" || head2.expression.type !== "ObjectExpression") {
      ...
      const error = new VFileMessage("Unexpected `" + head2.type + "` in code: expected an object spread (`{...spread}`)", {
        place: place.start,
        ruleId: "non-spread",
        source: "micromark-extension-mdx-expression"
      });
    ...
    if (head2.expression.properties[0] && head2.expression.properties[0].type !== "SpreadElement") {
      ...
      const error = new VFileMessage("Unexpected `" + head2.expression.properties[0].type + "` in code: only spread elements are supported", {
        place: place.start,
        ruleId: "non-spread",
        source: "micromark-extension-mdx-expression"
      });
...
function zwitch(key2, options) {
  ...
  function one3(value, ...parameters) {
    ...
    if (value && own6.call(value, key2)) {
      ...
      fn = own6.call(handlers2, id) ? handlers2[id] : one3.unknown;
    }
    if (fn) {
...
function defaultUnknownHandler(state, node2) {
  ...
  const result = "value" in node2 && !(own7.call(data2, "hProperties") || own7.call(data2, "hChildren")) ? { type: "text", value: node2.value } : {
    type: "element",
    tagName: "div",
    properties: {},
    children: state.all(node2)
...
var handle2 = zwitch("type", {
  invalid,
  unknown,
  handlers: { comment, doctype, element, raw, root: root3, text: text6 }
});
...
// src/lib/component-renderers.mjs
...
var UnknownComponentError = class extends Error {
  constructor(name2) {
    super(`\u672A\u77E5\u7EC4\u4EF6 <${name2}>\u3002\u53EF\u7528\u7EC4\u4EF6\uFF1A${Object.keys(renderers).join(", ")}`);
...
var _uidCounter = 0;
function uid(prefix) {
  return `${prefix}-${Date.now().toString(36)}-${_uidCounter++}`;
}
var CALLOUT_TYPES = ["tip", "warning", "info", "error"];
...
function parseSteps(val) {
  ...
  try {
    const p2 = JSON.parse(String(val));
...
renderers.Timeline = function renderTimeline(props, _childrenHtml, snapshot) {
  ...
  const items = steps.map((step, i) => {
    ...
    const desc = step.description ? `<p style="${s.step_desc}">${escapeHtml(step.description)}</p>` : "";
    const bottomPad = isLast ? "" : ";padding-bottom:1.5rem";
    return `<div style="${s.step}${bottomPad}"><div style="display:flex;flex-direction:column;align-items:center"><div style="${s.step_number}">${i + 1}</div>${connector}</div><div style="${s.step_content}"><h4 style="${s.step_title}">${escapeHtml(step.title)}</h4>${desc}</div></div>`;
  }).join("");
...
renderers.CodeTabs = function renderCodeTabs(props, childrenHtml, snapshot) {
  ...
  return `<div class="fmx-code-tabs" data-component="code-tabs" data-id="${id}" data-props='${escapeHtml(dataProps)}' style="${s.wrapper}"><div class="fmx-tab-bar" style="${s.tab_bar}">${tabBarHtml}</div><div class="fmx-panels">${childrenHtml}</div></div>`;
};
...
renderers.ImageCarousel = function renderImageCarousel(props, _childrenHtml, snapshot) {
  ...
  const navHtml = images.length > 1 ? `<button type="button" class="fmx-nav-prev" style="${s.nav_button};${s.nav_button_prev}" data-role="prev">\u2190</button><button type="button" class="fmx-nav-next" style="${s.nav_button};${s.nav_button_next}" data-role="next">\u2192</button>` : "";
  const dotsHtml = images.length > 1 ? `<div style="${s.dots}">` + images.map(
    (_, i) => `<button type="button" style="${s.dot}${i === 0 ? ";" + s.dot_active : ""}" data-role="dot" data-index="${i}"></button>`
  ).join("") + `</div>` : "";
  const captionHtml = images[0].caption ? `<p data-role="caption" style="${s.caption}">${escapeHtml(images[0].caption)}</p>` : `<p data-role="caption" style="${s.caption};display:none"></p>`;
  const dataProps = JSON.stringify({ images });
  return `<div class="fmx-image-carousel" data-component="image-carousel" data-id="${id}" data-props='${escapeHtml(dataProps)}' style="${s.wrapper}"><div style="${s.frame}"><div style="position:relative">${imgs}</div>${navHtml}</div>` + captionHtml + dotsHtml + `</div>`;
};
renderers.Playground = function renderPlayground(props, _childrenHtml, snapshot) {
  ...
  const runBtn = output ? `<button type="button" data-role="run" style="${s.button_run}">\u25B6 \u8FD0\u884C</button>` : "";
...
async function renderJsxNode(node2, ctx) {
  const name2 = node2.name;
  if (!name2 || !renderers[name2]) {
    throw new UnknownComponentError(name2 || "(anonymous)");
  }
  const props = extractProps(node2);
  const childrenHtml = await renderChildrenToHtml(node2.children || [], ctx);
  const spec = ctx.snapshot?.components?.[name2];
  if (spec?.interactive && spec.js_behavior_key) {
    ctx.usedComponents.add(spec.js_behavior_key);
  }
  return renderers[name2](props, childrenHtml, ctx.snapshot);
}
...
async function transformNode(node2, ctx) {
  if (isJsxNode(node2)) {
    const html7 = await renderJsxNode(node2, ctx);
    return { type: "html", value: html7 };
  }
  if (node2.children && Array.isArray(node2.children)) {
    const newChildren = [];
    for (const c of node2.children) newChildren.push(await transformNode(c, ctx));
    return { ...node2, children: newChildren };
  }
  return node2;
}
async function renderTree(tree, ctx) {
  return renderChildrenToHtml(tree.children || [], ctx);
}

// src/lib/base-styles.mjs
function buildStyleBlock(snapshot) {
  const tokenCss = [];
  tokenCss.push(":root {");
...
function buildInteractiveScript(usedSet) {
  if (!usedSet || usedSet.size === 0) return "";
  const fns = [];
  const handlerLines = [];
  for (const key2 of usedSet) {
    if (!BEHAVIORS[key2]) continue;
    fns.push(BEHAVIORS[key2]);
    const fnName = "bind" + key2.split("-").map((s) => s[0].toUpperCase() + s.slice(1)).join("");
    handlerLines.push(`    '${key2}': ${fnName}`);
  }
...
async function main() {
  ...
  const snapshotPath = process.env.FEIMA_SNAPSHOT || join(__dirname, "..", "references", "feima-style-snapshot.json");
...
${scriptBlock}
  ...
  await writeFile(outPath, html7, "utf8");
  console.log(outPath);
}
main().catch((e) => {
